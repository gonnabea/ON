...
export const socketController = (socket) => {
  return (
    ...
    socket.on(events.sendMsg, ({newMessage, roomID,roomID2}) => {
      ...
      console.log(`룸 ID: ${roomID}`)
      socket.in(roomID).in(roomID2).emit(events.sendMsg, newMessage) // 송신자 제외 채팅방 내의 클라이언트에게 채팅메세지 전송
      
    }),
    socket.on(events.leaveRoom, ({roomID, roomID2}) => {
      // 채팅방 퇴장 시 채널 제거
      ...
      socket.leave(roomID).leave(roomID2) 
    })
  )
}
